<template>
  <div class="container">
    <div class="notebook-wrapper" :style="{ height: notebookHeight + 'px' }">
      ...
      <!-- Текст-подсказка -->
      <div class="hint-text">
        сделай скриншот и потом нажми сюда ->
      </div>

      <!-- Кнопка Telegram в левом нижнем углу -->
      <button class="telegram-button" @click="sendToTelegram">
        <img src="/src/assets/images/logoTG1.png" alt="Telegram" class="telegram-icon" />
      </button>
...
// Массив случайных прилагательных для учителя
const teacherAdjectives = [
  'Amazing', 'Awesome', 'Brilliant', 'Cool', 'Crazy', 'Vincenik',
  'Fantastic', 'Funny', 'Great', 'Incredible', 'Lovely', 'Marvelous',
  'Super-puper', 'Phenomenal', 'Nice', 'Rock`n`roll', 'Super',
  'Handsome', 'Wonderful', 'The best', 'Favorite', 'Smart', 'Kind'
]

// Функция для получения случайного прилагательного
const getRandomAdjective = () => {
  const randomIndex = Math.floor(Math.random() * teacherAdjectives.length)
  return teacherAdjectives[randomIndex]
}

// Функция для отправки сообщения в Telegram
const sendToTelegram = () => {
  const adjective = getRandomAdjective()
  const message = `${adjective} teacher! Check my answers, please: (on screenshot)`
...
.message-input {
  ...
  /* Внутренние отступы */
  padding: 20px 0 20px 20px; /* Вот они - 20px сверху и снизу */
...
.hint-text {
  ...
  /* Едва заметный эффект при наведении на кнопку */
  .telegram-button:hover + & {
    color: rgba(128, 128, 128, 0.8);
  }
}

/* Обновленная кнопка Telegram - более аккуратная */
.telegram-button {
  position: absolute;
  bottom: 10px;
  right: 10px;
