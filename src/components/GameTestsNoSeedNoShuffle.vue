...
<!--  OLD NAME OF THIS FILE brainStormExercise-->
  <div v-if="exerciseData" class="exercise-container">
    ...
    >
      ...
      <div class="task-content">
        ...
        <!-- Multiple Choice с историей ответов -->
        <div v-else-if="task.taskID === 'multiple_choice'" class="multiple-choice-container">
          <div v-for="(q, qi) in task.questions" :key="qi" class="question-container">
            ...
            <div v-if="answers[index][qi]" class="explain-container">
              ...
              >
                {{ expandedExplanations[index]?.[qi] ? 'Ну ладно... ' : 'Показать объяснение' }}
              </button>
            </div>
          </div>
        </div>

        <!-- Student Input блок с авто-растягивающимися textarea -->
        <div v-else-if="task.taskID === 'student_input'" class="student-input-container">
          <div v-for="(q, qi) in task.questions" :key="qi" class="question-container">
            ...
            <span v-if="q.correctAnswer === 'anyIsOk'"
                  class="correct-answer">✓ Принимается любой ответ</span>
      ...
      <span v-for="(item, key) in feedback.incorrect" :key="key" class="incorrect-feedback">
        ...
        <div class="task-footer">
          ...
          <div class="task-controls">
            ...
            <!-- Показываем проценты ТОЛЬКО для заданий, которые не multiple choice -->
            <div
              v-if="task.taskID !== 'multiple_choice'"
              class="score-display"
              :class="{ 'score-visible': checkedTasks[index] }"
            >
              {{ checkedTasks[index] ? `${taskScores[index]}%` : '' }}
            </div>

            <!-- Кнопка Проверить для всех заданий, кроме discussion_task и multiple_choice -->
            <button
            ...
            <!-- Кнопка Shuffle только для discussion_task -->
            <button
...
import gameTestsNoSeedNoShuffleData from '../dataForGames/GameTestsNoSeedNoShuffleData'
...
// НОВАЯ ФУНКЦИЯ: Обработка выбора с историей для multiple choice
  ...
  if (task.taskID === 'grid_table') {
    ...
    task.questions.forEach((q, qi) => {
      if (task.taskID === 'student_input') {
        ...
        // Получаем все dropdown поля из этого вопроса
        const questionParts = splitDropDownText(q.text)
        let allDropdowns = []
        questionParts.forEach(paragraph => {
          paragraph.forEach(part => {
            if (part.type === 'dropdown') {
              allDropdowns.push(part)
            }
          })
        })

        // Проверяем каждый dropdown в этом вопросе
        let questionCorrect = true
        allDropdowns.forEach(part => {
        ...
        // Если все dropdown в вопросе правильные - засчитываем вопрос
        if (questionCorrect) {
          correctCount++
        }
        totalQuestions++
...
// НОВАЯ ФУНКЦИЯ: Получение класса для опции с учетом истории
const getInstantOptionClass = (taskIndex, questionIndex, optionValue, correctAnswer) => {
  ...
  // Если ответ правильный и был когда-либо выбран - он всегда зеленый!
...
const isDropDownAnswerCorrect = (taskIndex, questionIndex, originalText) => {
  ...
  // Получаем все части текущего вопроса
  const questionParts = splitDropDownText(originalText)

  // Собираем все dropdown поля из этого вопроса
  let allDropdowns = []
  questionParts.forEach(paragraph => {
    paragraph.forEach(part => {
      if (part.type === 'dropdown') {
  ...
  const allAnswered = allDropdowns.every(part => {
    const answer = answers.value[taskIndex][questionIndex][part.index]
  ...
  const allCorrect = allDropdowns.every(part => {
...
const checkGridTableAnswers = (taskIndex) => {
  ...
  task.tableData.rows.forEach((row, ri) => {
    const rowFeedback = {
      allCorrect: true,      // все ответы точные
      hasAlmost: false,      // есть almost ответы
      hasErrors: false,      // есть ошибки
    ...
    row.editable.forEach((isEditable, ci) => {
      ...
      if (almostCorrect && almostCorrect.some(alt => normalizeAnswer(alt) === userAnswer)) {
        ...
        rowFeedback.allCorrect = false // строка не считается полностью правильной для отображения
        // НО для подсчета процентов оставляем isCorrectForScoring = true
        return // almost correct, но не ошибка
      }

      // Если ни то, ни другое - ошибка
      rowFeedback.allCorrect = false
      rowFeedback.isCorrectForScoring = false // для процентов - ошибка
  ...
  feedback.forEach(row => {
    if (row.isCorrectForScoring) correctCount++ // точные ИЛИ almost считаем правильными
  })

  const percentage = Math.round((correctCount / task.tableData.rows.length) * 100)
  return percentage
}
...
onMounted(() => {
  ...
  exerciseData.value = gameTestsNoSeedNoShuffleData[currentMission.value] || null

  if (exerciseData.value) {
    // Сохраняем оригинальный порядок и перемешиваем вопросы для discussion_task
    ...
    // НОВОЕ: Инициализируем историю ответов
    selectedAnswersHistory.value = exerciseData.value.tasks.map(task => {
    ...
    expandedExplanations.value = exerciseData.value.tasks.map(task =>
      task.taskID === 'multiple_choice' ? Array(task.questions.length).fill(false) : null
    )
...
const rainbowColors = [
  '#cff0ff', '#e0c0ff', '#ffc0cb', '#ffd700', '#98fb98', '#ffa07a', '#ff6347'
]
</script>
...
<style scoped>
/* ========== ОСНОВНЫЕ СТИЛИ (из второго кода) ========== */
...
/* Добавь в раздел Grid Table стилей */
.almost-feedback {
  color: #f59e0b;
  font-weight: 500;
  margin-left: 5px;
...
/* ========== НОВЫЕ СТИЛИ ИЗ ПЕРВОГО КОДА ========== */

/* Стили для истории выбора в multiple choice */
...
.explanation-content {
  ...
  border-radius: 4px;
  font-size: 0.9rem;
  white-space: pre-line;  /* 👈 ДОБАВЬ ЭТУ СТРОЧКУ ДЛЯ ОТОБРАЖЕНИЯ \n в данных */
  color: #1f2937;
}
